...
name: prompt-master
description: >
  Reference database of prompt engineering frameworks, tool-specific syntax
  rules, and 35 documented anti-patterns for writing prompts that work on the
  first try. Contains critical information you need to look up before writing
  prompts: Midjourney requires comma-separated descriptors not prose, o1/o3
  models degrade with Chain of Thought instructions, Stable Diffusion needs
  (word:weight) syntax and mandatory negative prompts, Claude Code agents need
  explicit stop conditions, Cursor/Copilot need file-scope anchoring. Use this
  skill whenever the user wants to write a prompt for any AI tool, craft a
  system prompt, fix a prompt that's producing bad results, optimize token usage
  in prompts, or asks how to phrase something for
  ChatGPT/GPT-4/Gemini/Claude/o1/o3/Cursor/Copilot/Midjourney/DALL-E/Stable
  Diffusion/Sora/ElevenLabs. The skill selects from 9 frameworks (RTF, CO-STAR,
  RISEN, CRISPE, CoT, Few-Shot, File-Scope, ReAct, Visual Descriptor) based on
  the task type — you cannot replicate this selection logic without consulting
  the skill.
generated-status: failed
verified-at: '2026-03-17T02:29:53.463Z'
...
"Use only information you are highly confident is accurate. If you're uncertain about any aspect, say 'I don't have enough information' rather than guessing. If uncertain, write [uncertain] next to the claim. Do not fabricate citations or statistics."

**Direct-quote extraction** — for long-document analysis tasks (>20K tokens):
"First, extract exact word-for-word quotes from the document that are relevant to the task. Then perform your analysis using only those quotes. If no quote supports a claim, remove the claim and mark where it was removed with empty [] brackets."
