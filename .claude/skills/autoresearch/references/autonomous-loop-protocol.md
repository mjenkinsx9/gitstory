...
**Metric extraction must succeed.** If the extracted metric is blank, empty, null, or non-numeric:
1. Print: `ERROR: Verify command did not produce a parseable metric. Got: "<output>", expected: <number>`
2. Do not proceed with the iteration — this is a **crash** (invalid verification, not a metric failure)
3. Re-examine the verify command: is the tool installed? Is the extraction pipeline correct? Fix and retry (max 3 attempts)
4. If unrecoverable after 3 attempts, log "crash" and move to next iteration

**This is different from "metric did not improve"** — that is a normal keep/discard outcome. Here, the verify command itself failed to execute or produce a number.
