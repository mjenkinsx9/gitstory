...
1. **Dry run** the verify command on current codebase — actually execute it with Bash
2. Confirm it exits with code 0
3. **Extract the metric number** from the output — if the number is blank, empty, or not parseable, the verify command is INVALID
4. Record the baseline metric value
5. If dry run fails or metric is not parseable -> show the error or blank output, ask user to fix, re-validate
...
**Do not proceed if verify command fails dry run.** Help user fix it. **Do not print "requires dry-run" and skip the execution — actually run the command.**
